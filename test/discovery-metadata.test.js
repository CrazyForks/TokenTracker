...
const README_EXPECTATIONS = [
  ["README.md", /28 AI coding tools/, /\|\s+\*\*AI tools supported\*\*\s+\|\s+\*\*28\*\*/],
  ["README.zh-CN.md", /28 款 AI 编码工具/, /\|\s+\*\*支持的 AI 工具数\*\*\s+\|\s+\*\*28\*\*/],
  ["README.ja.md", /28 種類の AI コーディングツール/, /\|\s+\*\*対応 AI ツール数\*\*\s+\|\s+\*\*28\*\*/],
  ["README.ko.md", /28개의 AI 코딩 도구/, /\|\s+\*\*지원 AI 도구\*\*\s+\|\s+\*\*28\*\*/],
  ["README.de.md", /28 KI-Coding-Tools/, /\|\s+\*\*Unterstützte KI-Tools\*\*\s+\|\s+\*\*28\*\*/],
];

test("public discovery surfaces describe all 28 supported tools", () => {
  ...
  assert.match(index, /Supported AI coding tools \(28\)/);
  ...
  assert.match(llms, /Supported AI coding tools \(28\)/);
  assert.match(llms, /desktop pet/i);
  assert.match(llms, /four desktop widgets/i);
  assert.match(llms, /achievements/i);
});

test("marketing logo wall includes the same 28 product integrations", () => {
  ...
  assert.equal(new Set(providers).size, 28);
...
test("CLI onboarding advertises the same 28 supported integrations", () => {
...
test("npm metadata carries the current product hook", () => {
  ...
  assert.match(pkg.description, /28 tools/);
