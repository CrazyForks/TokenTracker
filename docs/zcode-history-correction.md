...
首次适用的同步会先备份原有队列，再重建 ZCode 统计并在账号同步时覆盖原有历史桶；不会修改 ZCode 原始数据库，也不会修改其他来源的用量。迁移完成时间记录在本地 `cursors.json` 的 `zcodeInclusiveTokenRepair_2026_09` 中，重复同步不会再次运行此迁移。如有无法解释的差异，请保留本地数据库及 `*.bak.*` 备份，仅提交版本、日期和去敏后的汇总计数，不要公开完整日志、数据库或凭据。
